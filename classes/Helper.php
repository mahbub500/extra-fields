 ...
 */
namespace Codexpert\ExtraFields;
...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "ef_{$_args['post_type']}", 'ef' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "ef_{$_args['post_type']}", $posts, 'ef', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'ef' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'ef_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /extra-fields directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with extra-fields_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'ef_template_overwrite_dir', get_stylesheet_directory() . '/extra-fields/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( Extra_Fields ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'extra-fields' );
		}
	}
}